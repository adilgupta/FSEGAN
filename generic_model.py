...
class generic_model(nn.Module):
    ...
    def save_model(self, is_best, epoch, train_loss, test_loss, test_acc, rnn_name, layers, hidden_dim):

        base_path = self.config_file['dir']['models']
        if is_best:
            filename = base_path + 'best_' + '_'.join([rnn_name, str(layers), str(hidden_dim)]) + '.pth'
        else:
            filename = base_path + str(epoch) + '_'.join([rnn_name, str(layers), str(hidden_dim)]) + '.pth'
    ...
    def load_model(self, mode, rnn_name, layers, hidden_dim, epoch=None):
        ...
        # train
            ...
            if epoch is not None:
                ...
                filename = self.config_file['dir']['models'] + str(max(to_check)) + '_'.join(
                    [rnn_name, str(layers), str(hidden_dim)]) + '.pth'
